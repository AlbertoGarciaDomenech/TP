...
public class Board {
	...
	public void shootLaser(int row,int y, int damage) {
		for(int i = y; i < COLS;i++) {
			if(zombieList.isGameObject(row, i)) {
				takeZombieDamage(row, i, damage);
			}
		}
	...
	public void cactusReturnAttack(int x, int y, int damage) {
		if(zombieList.isGameObject(x, y)) {
		zombieList.takeDamage(x, y+1, damage);}
	}

	public void mageRage(int damage) {
		int random = new Random().nextInt(plantList.getCont());
		int x = plantList.getXfromPos(random);
		int y = plantList.getYfromPos(random);
		plantList.takeDamage(x, y, damage);
	}

	public boolean canMageRage() {
		return plantList.getCont() > 0;
	}

	public String delete(int x, int y) {
		return plantList.delete(x, y);
	}
	public String deleteZombie(int x, int y) {
		return zombieList.delete(x, y);
	}

	public void setShield(int x, int y) {
		plantList.setShield(x,y);
	}
