...
//		cycleCounter = 0;
	...
	public boolean addPlantToGame(Plant plant, int x, int y) throws CommandExecuteException{
		if(x >= 0 && x < getRows() && y >= 0 && y < getCols()-1 && isEmpty(x,y)) {
			try {
	...
	public void computerAction() {
		if(zombieManager.isZombieAdded()) {
			...
			zombie.setCoords((cycleCounter % (board.getRows())), (board.getCols()-1));
			zombie.setGame(this);
	...
	public void laser(int row,int y, int damage) {
		if(!board.emptyRow(row)) 
		{
			board.shootLaser(row,y,damage);
		}
	}
	
	public void tripleShoot(int row, int damage) {
		for(int i = -1; i < 2; i++) {
			if(row + i >= 0 && row + i  < 4) {
				if(!board.emptyRow(row+i)) 
				{
					int y = board.firstZombie(row+i);
					board.takeZombieDamage(row+i,y, damage);
				}
			}
		}
	}
	...
	public void cactusReturnAttack(int x, int y, int damage) {
		board.cactusReturnAttack(x,y,damage);
	}
	...
	public void mageRage(int damage) {
		board.mageRage(damage);
	}

	public boolean canMageRage() {
		return board.canMageRage();
	}

	public void delete(int x, int y) {
		String plantName = board.delete(x,y);
		Plant plant = PlantFactory.getPlant(plantName);
		suncoinManager.addCoins(plant.getCost()/2);
	}

	public boolean suddenDeath(int x, int y, int cost) throws CommandExecuteException {
		try {
			suncoinManager.spendCoins(cost);
			String zombieName = board.deleteZombie(x,y);
			return true;
	...
	public boolean setShield(int x, int y, int cost) throws CommandExecuteException {
		try {
			suncoinManager.spendCoins(cost);
			board.setShield(x,y);
			return true;
