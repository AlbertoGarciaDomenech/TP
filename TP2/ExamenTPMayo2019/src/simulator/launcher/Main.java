...
import simulator.view.DeadZoneCollision;
...
public class Main {
	...
	private final static Double _radioDefaultValue = (double) 200;
	...
	private static void parseDeadZoneOption(CommandLine line) throws ParseException {
		String s = line.getOptionValue("dzone", _radioDefaultValue.toString());
		try {
			_r = Double.parseDouble(s);
			assert (_r > 0);
		} catch (Exception e) {
			throw new ParseException("Invalid dead zone value: " + s);
		}		
	}
	...
	private static Options buildOptions() {
		...
		//deadzone radio
		cmdLineOptions.addOption(Option.builder("dzone").longOpt("deadzone").hasArg()
				.desc("An integer representing the radio of\r\n" + 
				"Collision. Default value: " + _radioDefaultValue +
				"\r\n").build());
		// help
...
//			return;
	...
	private static void startBatchMode() throws Exception {
		...
		DeadZoneCollision _dz = new DeadZoneCollision(_ctrl, _r);
		_sim.addObserver(_dz);
		
		_ctrl.run(_steps, output);
		System.out.println(_dz.getCollision());
	}
